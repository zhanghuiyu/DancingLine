...
public class LineController_Spring : MonoBehaviour {
    ...
    void Start () {
        ...
        lineMeshRender.material.color = Color.red;
        cubeMeshRender = cubeGameObject.GetComponent<MeshRenderer>();
        cubeMeshRender.material.color = Color.red;

        //動態生成Line
        shareData_Spring.trackCubePosition = transform.localPosition; //將trackCubePosition設為Line的起點
        InvokeRepeating("createLine",0.0f, shareData_Spring.movingSpeed);
	}

    void createLine()
    {
        if (shareData_Spring.gameStart && cubeGameObject != null)
        {
            ...
            newCubeGameObject.transform.localPosition = shareData_Spring.trackCubePosition + direction;
            shareData_Spring.trackCubePosition = newCubeGameObject.transform.localPosition;
            /*之後要記得補上把Cube刪掉的部分*/
    ...
    void Update () {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (!shareData_Spring.gameStart)
            {
                BGM.Play(); //撥放音樂
                shareData_Spring.timer.Start(); //按下計時
                shareData_Spring.gameStart = true; //遊戲開始
            }
